...
# ── Выбор стратегии ──────────────────────────
# "bounce" — отскок от уровней поддержки/сопротивления
# "trend"  — trend-following: вход по тренду на откате к EMA20
# "auto"   — обе стратегии работают параллельно:
#            - если обе сигналят в одну сторону → берём с высшей силой (consensus)
#            - если в разные стороны → пропускаем (неопределённость)
#            - если одна молчит → берём от активной
#            Каждая сделка тегируется (trend / bounce / consensus) для аналитики.
STRATEGY_MODE = "auto"
